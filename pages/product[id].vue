          ...
          <div v-if="product.images && product.images.length > 1" class="grid grid-cols-4 gap-2">
            <button
              ...
              :class="[
                ...
                selectedImage === image ? 'border-red-600' : 'border-gray-200 hover:border-gray-400'
              ]"
            >
...
const fetchProduct = async () => {
  ...
  try {
    ...
    if (data.images && data.images.length > 0) {
      ...
      selectedImage.value = '/placeholder.jpg'
    }
  } catch (err) {
...
const fetchSimilarProducts = async () => {
  ...
  try {
    ...
    similarProducts.value = data.products.filter(p => p._id !== product.value._id)
  } catch (err) {
